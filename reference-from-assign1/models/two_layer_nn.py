...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        if type(X) is list:
            X = np.array(X)
        N = X.shape[0]
        p1 = np.dot(X, self.weights['W1']) + self.weights['b1']
        p2 = self.sigmoid(p1)
        p3 = np.dot(p2, self.weights['W2']) + self.weights['b2']
        x_preds = self.softmax(p3)
        ...
        dW2 = np.dot(p2.T, dp3)
        db2 = np.sum(dp3, axis=0)

        dp2 = np.dot(dp3, self.weights['W2'].T)

        dp1 = dp2 * self.sigmoid_dev(p1)
